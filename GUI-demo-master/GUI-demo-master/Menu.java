...
public class Menu extends Actor
{
     ...
     * handleMouse
     * 
     * Controls the click detection for the drop-down menu, and the additional menu buttons.
     * 
    ...
    private void handleMouse()
    {
        ...
        if (Greenfoot.mouseClicked(titleBar) == true)
        {
            if (visible == false)
            {
                ((UIMainWorld)getWorld()).addObject(menuItems,getX(),getY()+(titleHeight + menuHeight)/2);
            }
            else
            {
                ((UIMainWorld)getWorld()).removeObject(menuItems);
            }
            visible = !visible;
        }
